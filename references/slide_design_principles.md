...
- ✅ Concise but informative text (audiences read OR listen, not both simultaneously)
- ✅ Clear visual focus
...
**Guideline**: MOST lines should be 6-8 words (staying on one line in the slide). Some lines can exceed this when clinical detail requires it. Lists of etiology, risk factors, investigations, differential diagnoses, medications, etc. should be presented as bullet points.

**Rationale**:
- Too much text = audience reads instead of listens
- Too little text = slides lack useful reference content
- Medical content often requires lists that should be bulleted, not compressed

**Best Approach**:
- 6-8 words per line is the target for MOST lines
- Some lines can go longer (e.g., clinical descriptions, trial results)
- Use fragments, not complete sentences
- Consider replacing text with visuals where possible
...
1. **Title slide**: Presentation title + [Author] at the bottom (no subtitle, institution, affiliation, or date)
...
6. **Closing**: "Thank You / Questions & Discussion" (no contact info or references)
...
- [ ] Most lines 6-8 words; lists (etiology, risk factors, etc.) bulleted
