...
python skills/medical-pptx/scripts/pdf_to_images.py presentation.pdf slide --dpi 150
...
python skills/medical-pptx/scripts/pdf_to_images.py presentation.pdf slide --dpi 300

# PNG format (lossless, larger files)
python skills/medical-pptx/scripts/pdf_to_images.py presentation.pdf slide --dpi 150 --format png
```

**Convert Specific Slides**:
```bash
# Slides 5-10 only
python skills/medical-pptx/scripts/pdf_to_images.py presentation.pdf slide --dpi 150 --first 5 --last 10

# Single slide
python skills/medical-pptx/scripts/pdf_to_images.py presentation.pdf slide --dpi 150 --first 3 --last 3
```

**Output Options**:
```bash
# Different output directory
python skills/medical-pptx/scripts/pdf_to_images.py presentation.pdf review/slide --dpi 150

# Custom naming
python skills/medical-pptx/scripts/pdf_to_images.py presentation.pdf output/presentation --dpi 150
```

### Method 2: Converting .pptx to Images (LibreOffice + PyMuPDF)

For PowerPoint presentations, convert to PDF first using LibreOffice, then to images:

```bash
# Step 1: Convert .pptx to PDF (requires LibreOffice)
libreoffice --headless --convert-to pdf presentation.pptx

# Step 2: Convert PDF pages to images using pdf_to_images.py
python scripts/pdf_to_images.py presentation.pdf slide --dpi 150
```

This two-step pipeline is the same approach used by Anthropic's pptx skill for visual QA.
