...
This guide provides comprehensive instructions for creating professional scientific presentations using PowerPoint, with emphasis on programmatic creation using PptxGenJS and best practices for scientific content.
...
- **Concise but informative text**: MOST lines 6-8 words (staying on one line), but some lines can go longer when clinical detail requires it. Lists of etiology, risk factors, investigations, medications, etc. should be bullet points.
...
- **PptxGenJS API**: `references/pptxgenjs_reference.md`

### Programmatic Creation (PptxGenJS)

**Best for**: Creating presentations from scratch with custom designs and data visualizations.

**Workflow**:
1. Read `references/pptxgenjs_reference.md` for the full API
2. Create JavaScript file using PptxGenJS API
3. Add charts, tables, images, icons, and shapes
4. Validate visually (convert to PDF, then to images)
...
- MOST lines 6-8 words (staying on one line in the slide)
- Some lines can exceed 8 words when clinical detail requires it
- Lists (etiology, risk factors, investigations, differentials, medications) should be presented as bullet points
- Avoid walls of text — aim for white space and readability

### 3. Color Schemes

**Selecting Colors**:

Consider your subject matter and audience — avoid defaulting to blue/navy. Many topics have stronger associations with other colors:
- **Academic/Professional**: Forest green, charcoal, or berry — not just navy
- **Biomedical**: Warm tones (terracotta, coral) or greens (sage, forest) — avoid red-green combinations
- **Technology**: Bold modern colors (coral, gold, purple)
- **Clinical**: Sage, eucalyptus, or warm neutrals — conservative doesn't mean blue

**Example Palettes**:

**Forest & Moss** (natural, growth):
- Background: Cream (#F5F5F5)
- Title: Forest green (#2C5F2D)
- Text: Charcoal (#2C2C2C)
- Accent: Moss (#97BC62)

**Warm Terracotta** (inviting, grounded):
- Background: Sand (#E7E8D1)
- Title: Terracotta (#B85042)
- Text: Dark brown (#3E2723)
- Accent: Sage (#A7BEAE)

**Berry & Cream** (elegant, distinctive):
- Background: Cream (#ECE2D0)
- Title: Berry (#6D2E46)
- Text: Charcoal (#2C2C2C)
- Accent: Dusty rose (#A26769)
...
- **Biotechnology/Life Sciences**: Forest (#2C5F2D), Moss (#97BC62), Cream (#F5F5F5)
...
- **Physics/Engineering**: Charcoal (#36454F), Off-white (#F2F2F2), Black (#212121)
- **Medicine/Healthcare**: Terracotta (#B85042), Sand (#E7E8D1), Sage (#A7BEAE)
- **Environmental Science**: Sage (#87A96B), Terracotta (#E07A5F), Cream (#F4F1DE)

See full palette options in `references/slide_design_principles.md`.
...
const DESIGN = {
  colors: {
    primary: "2C5F2D",    // Forest green (natural, grounded)
    accent: "97BC62",     // Moss (fresh, complementary)
    text: "2C2C2C",       // Charcoal (readable)
    background: "F5F5F5"  // Cream (warm, clean)
  },
  fonts: {
...
addTitleSlide(
  pptx,
  "Research Title",
  "[Author]"  // Title + author only — no subtitle, institution, or date
);
...
### Generate Slide Images for Review

After creating presentation, convert to images for visual inspection:

```bash
# Convert .pptx to PDF first (requires LibreOffice)
...
- ✅ Keep text concise: MOST lines 6-8 words, some can go longer; use bullet points for lists
...
- `references/pptxgenjs_reference.md`: PptxGenJS API reference
